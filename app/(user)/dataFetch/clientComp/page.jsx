...
export default function Page() {  // Capitalized name
  ...
  useEffect(() => {
    if (!userName) return
    const genderReveal = async () => {
      const API = await fetch(`https://api.genderize.io/?name=${userName}`)
      ...
      <div className="max-w-sm mx-auto bg-white rounded-2xl shadow-lg p-6 space-y-4">
        <div className="flex flex-col items-center space-y-2">
          ...
          <h2 className="text-lg text-center font-semibold text-gray-700">
            No Name Found. <br /> Please Provide a Name like <br />url?name=Your name
          </h2>
        </div>
      </div>
    )
  ...
  let confirmProbability = userInfo.probability ? userInfo.probability * 100 : 0
  ...
  return (
    <div className={`max-w-sm mx-auto ${userInfo.gender === 'male' ? 'bg-white' : 'bg-pink-300'} rounded-2xl shadow-lg p-6 space-y-4`}>
      ...
      <div className="flex flex-col items-center space-y-2">
        ...
        <span className={`bg-blue-100 ${userInfo.gender === 'male' ? 'text-blue-700' : 'text-pink-700'} text-sm font-medium px-3 py-1 rounded-full`}>{gender}</span>
      </div>
